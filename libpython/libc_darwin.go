...
func formatTM(formatRunes []rune, tmv *Ttm) string {
	...
	date := time.Date(int(tmv.Ftm_year)+1900, time.Month(tmv.Ftm_mon)+1, int(tmv.Ftm_mday), int(tmv.Ftm_hour), int(tmv.Ftm_min), int(tmv.Ftm_sec), 0, time.FixedZone("", int(tmv.Ftm_gmtoff)))
	...
	for i := 0; i < len(formatRunes); {
		...
		case 'Z':
			if tmv.Ftm_zone != 0 {
				value = libc.GoString(tmv.Ftm_zone)
			}
		default:
			value = "%" + string(verb)
		}
